...
def test_exception_type(exception):
    ...
    assert issubclass(getattr(exceptions, exception), exceptions.BaseException)
    assert isinstance(getattr(exceptions, exception)(), exceptions.BaseException)


def test_validation_error_type():
    assert issubclass(getattr(exceptions, validation_error), exceptions.BaseException)

    error = getattr(exceptions, validation_error)("field", "1", ValueError("It must be int"))
    assert isinstance(error, exceptions.BaseException)
